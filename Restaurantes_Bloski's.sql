...
CREATE TABLE `Turnos` (
  ...
  KEY `id_restaurante` (`id_restaurante`),
  CONSTRAINT `turnos_ibfk_1` FOREIGN KEY (`id_restaurante`) REFERENCES `Restaurante` (`id_restaurante`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
...
(7, 1, 'Agua de Valencia', 'Mezclamos jugo de naranja fresco con cava, ginebra y vodka. Añadimos azúcar al gusto y servimos bien frío en una jarra con hielo y rodajas de naranja.', 50, '12.50'),
(8, 2, 'Bacon Cheese Fries', 'Freímos las papas hasta que estén doradas y crujientes. Las cubrimos con queso cheddar rallado y bacon crujiente troceado. Gratinar hasta que el queso se derrita. Servir con salsa ranch.', 50, '5.99');
